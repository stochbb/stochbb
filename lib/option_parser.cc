...
KeywordRule::KeywordRule(Parser *parser, const std::string &keyword)
...
{
  std::list<RuleInterface *>::iterator it = this->options.begin();
...
ValueRule::ValueRule(Parser *parser, const std::string &identifier)
...
Parser::format_help(std::string prg_name) {
  std::stringstream out;
  out << "Usage: " << prg_name << " " << _rule->dump() << std::endl;

  return out.str();
}

void
Parser::set_flag(const std::string &name) {
...
void
Parser::set_option(const std::string &name, const std::string &value) {
...
void
Parser::add_value(const std::string &identifier, const std::string &value) {
...
bool
Parser::has_keyword(const std::string &keyword) const {
  return (0 != _present_keywords.count(keyword));
}

void
Parser::add_keyword(const std::string &keyword) {
