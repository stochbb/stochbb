...
In the following section, I outline the representation of random variables and the reductions performed on a network. 
In section \ref{sec:arch}, I describe the architecture of the framework followed by a in-detail description of the graphical
user interface (GUI) application that allows to assemble and analyze networks of dependent random variables easily. In section
\ref{sec:pod} I showcase the usage of the GUI application for an analysis of a simple response latency model. Finally, in section
\ref{sec:data} I describe how parameters of the distributions of random variables can be estimated from data.
